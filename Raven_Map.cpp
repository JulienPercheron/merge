...
//------------------------------------------------------------------------------------------------------------------New
void Raven_Map::AddWeapon_GiverDeath(int type_of_weapon, Vector2D pos, Raven_Team* team, int graphNode) {
    Trigger_WeaponGiver* wg = new Trigger_WeaponGiver(pos, team, graphNode);

    wg->SetEntityType(type_of_weapon);

    wg->SetRespawnDelay(99999999);
    //add it to the appropriate vectors
    m_TriggerSystem.Register(wg);
