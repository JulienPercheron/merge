...
void Raven_Bot::Update()
{
  ...
  /*if (this != nullptr && this->ID() == 404 && this->m_pTargSys->GetTarget() && this->m_pTargSys->GetPrimaryTarget())
	debug_con << "CURRENT TARGET OF " << this->ID() << "-->>" << this->m_pTargSys->GetTarget()->ID()
			  << " ; PRIMARY TARGET --> " << this->m_pTargSys->GetPrimaryTarget()->ID() << "";
  */
...
bool Raven_Bot::HandleMessage(const Telegram& msg)
{
  ...
  case Msg_YouGotMeYouSOB:
  {
	  ...
	  Raven_Bot* deadBot = (Raven_Bot*)msg.ExtraInfo;

	  std::list<Raven_Bot*>::const_iterator curBot = this->m_pWorld->GetAllBots().begin();
	  for (curBot; curBot != this->m_pWorld->GetAllBots().end(); ++curBot)
	  {
		  Raven_Bot* botCourant = *curBot;

		  if (botCourant->GetTeam() && m_pTargSys->GetPrimaryTarget() && m_pTargSys->GetPrimaryTarget()->ID() == deadBot->ID()) {
			  botCourant->GetTeam()->Communicate_DeathOfTarget(this);
			  break;
		  }
	  }
  ...
  {
	  Raven_Bot* theTarget = (Raven_Bot*)msg.ExtraInfo;
	  int bossID = msg.Sender;

	  this->m_pTargSys->SetPrimaryTarget(theTarget);

	  debug_con << "---------------------" << "";
	  debug_con << bossID << " HAS ORDERED " << this->ID() << " TO KILL " << theTarget->ID() << "";
  ...
  {
	  this->m_pTargSys->ClearPrimaryTarget();

	  int killerID = msg.Sender;

	  debug_con << "---------------------" << "";
	  debug_con << killerID << " HAS KILLED THE PRIMARY TARGET OF " << this->ID() << ". CLEARING TARGET..." << "";
  ...
  case Msg_OffsetOn:
  {
	  int* followerCounter = (int*)msg.ExtraInfo;
	  int teamSize = this->GetTeam()->GetTeamSize()-1;
	  Vector2D offSet = Vector2D(sin(Pi * 2 * *followerCounter / teamSize) * 30, cos(Pi * 2 * *followerCounter / teamSize) * 30);
	  this->m_pSteering->OffsetPursuitOn(m_pWorld->PossessedBot(), offSet);
	  this->SetPos(this->m_pWorld->PossessedBot()->Pos() + offSet);
	  return true;
  }
  ...
  case Msg_OffsetOff:
  {
	  this->m_pSteering->OffsetPursuitOff();
	  return true;
  }
...
//--------------------------- Possess -----------------------------------------
//
//  Utilisé lorsqu'on charge la map, ignore les conditions de spawn ou de mort
//-----------------------------------------------------------------------------
void Raven_Bot::TakePossessionWhenLoadingMap()
{
	m_bPossessed = true;
...
void Raven_Bot::ChangeBotDrawColor() const {//-----------------------------------------Définition des couleurs, 0 = red, 1 = blue, 2 = yellow, 3 = green
	if (this->GetTeam()) {
		switch (this->GetTeam()->GetColor())
		{
		case(0):
			gdi->RedPen();
			break;
