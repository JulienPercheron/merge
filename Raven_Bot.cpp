...
void Write()
{
   ...
   // if (myFile.is_open() ){
       // for (int i = 0; i < m_trainingSet.GetOutputSet().size(); ++i) {
          //  myFile << "a\n";
          //  myFile << (m_trainingSet.GetOutputSet().at(1)) << " " << m_trainingSet.GetTargetsNb();
          //  myFile << (m_trainingSet.GetOutputSet().at(2)) << " " << m_trainingSet.GetTargetsNb();
