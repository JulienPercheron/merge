...
//Nouveau constructeur pour en créer à la mort d'un agent
//pos : position de l'agent, team : equipe de l'agent, graphNode : graph node de l'agent
Trigger_WeaponGiver::Trigger_WeaponGiver(Vector2D pos, Raven_Team* team, int graphNode):Trigger_Respawning<Raven_Bot>(1) {
    

    SetPos(Vector2D(pos.x, pos.y));
    SetBRadius(10);
    SetGraphNodeIndex(graphNode);
...
{
  if ((this->isActive()) && (this->isTouchingTrigger(pBot->Pos(), pBot->BRadius())) && ((teamSpecific == nullptr) || teamSpecific == pBot->GetTeam())) //Pour ne pas donner l'arme à quelqu'un d'une autre équipe
  {
