    ...
    case WM_KEYUP:
      {
        switch(wParam)
        {
		 ...
		 case 'O':
			 if (g_pRaven->PossessedBot() && g_pRaven->PossessedBot()->GetTeam())
			 {
				 Raven_Team* playerTeam = g_pRaven->PossessedBot()->GetTeam();
				 if (!playerTeam->IsInOffset()) {
					 playerTeam->ApplyOffsetPursuit();
				 }
				 else {
					 playerTeam->DisableOffsetPursuit();
				 }
			 }
