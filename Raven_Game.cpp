...
{ 
  ...
  for (curBot; curBot != m_Bots.end(); ++curBot)
  {
    ...
    else if ((*curBot)->isDead())
    {
      ...
      //-----------------------------------------------------------------------------------------------------------New
      int graphNode = (*curBot)->GetPathPlanner()->GetClosestNodeToPosition((*curBot)->Pos());
      int weapon_type = (*curBot)->GetWeaponSys()->GetCurrentWeapon()->GetType();
      //Can't create weapon giver of default weapon
      if(weapon_type != 9) {
          m_pMap->AddWeapon_GiverDeath(weapon_type, (*curBot)->Pos(), (*curBot)->GetTeam(), graphNode);
      }
...
//-------------------------- CreateTeam -------------------------------------- ---------------------------------------Nouvelle méthode
//
//  Adds a team in the team list
//-----------------------------------------------------------------------------
void Raven_Game::CreateTeam(const std::string& pName, unsigned int nbPlayers)
{
	if (nbPlayers > 0) {
		Raven_Team* nouvelleEquipe = new Raven_Team(this, pName, nbPlayers);
		this->m_Teams.push_back(nouvelleEquipe);

#ifdef LOG_CREATIONAL_STUFF
		debug_con << "Adding new team";
#endif
	}
}
...
{  
  ...
  { 
    ...
    if (script->GetInt("NumBots_RED") > 0) {
        CreateTeam("RED", script->GetInt("NumBots_RED")); //------------------------------- Ajout d'une team avec un joueur en plus
        this->m_Teams.back()->SetColor_RED();
    }
	

    if (script->GetInt("NumBots_BLUE") > 0) {
        CreateTeam("BLUE", script->GetInt("NumBots_BLUE"));
        this->m_Teams.back()->SetColor_BLUE();
    }
	

    if (script->GetInt("NumBots_YELLOW") > 0) {
        CreateTeam("YELLOW", script->GetInt("NumBots_YELLOW"));
        this->m_Teams.back()->SetColor_YELLOW();
    }
	

    if (script->GetInt("NumBots_GREEN") > 0) {
        CreateTeam("GREEN", script->GetInt("NumBots_GREEN"));
        this->m_Teams.back()->SetColor_GREEN();
    }
...
void Raven_Game::ClickLeftMouseButton(POINTS p)
{
  ...
  //Si on clique sur un bot, que le bot sélectionné est contrôlé par le joueur et si target c'est pas nous-même
  if (pBot && m_pSelectedBot && m_pSelectedBot->isPossessed() && m_pSelectedBot != pBot) {
	  if (m_pSelectedBot->GetTeam() != nullptr) {
		  m_pSelectedBot->GetTeam()->Communicate_NewTarget(pBot);
	  }
  }
}
