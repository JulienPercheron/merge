...
{ 
  ...
  for (curBot; curBot != m_Bots.end(); ++curBot)
  {
    ...
    else if ( (*curBot)->isAlive())
    {
	  ...
	  if ((m_TrainingSet.GetInputSet().size() < 250) )  {
          
		  //ajouter une observation au jeu d'entrainement
          if ((index >= 4) &&  (m_pSelectedBot && m_pSelectedBot->isPossessed())) {
...
//----------------------------- saveData -------------------------------
//-----------------------------------------------------------------------------
void saveData(std::string field) {
    std::ofstream file;
    file.open("Datas.csv", std::ios_base::app);
    file << field << std::endl;
}
...
bool Raven_Game::AddData(vector<double>& data, vector<double>& targets)
{
	if (data.size() > 0 && targets.size() > 0) {
		...
		if (data.size() == m_TrainingSet.GetInputNb() && targets.size() == m_TrainingSet.GetTargetsNb()) {
            ...
            {
                double elem = *it;
                message += to_string(elem);    //On accède à l'élément pointé via l'étoile
                message += ",";
            }
...
//-------------------------------- LoadTrainingSet ------------------------------------
//
//  sets up the training set from .csv file
//-----------------------------------------------------------------------------
bool Raven_Game::LoadTrainingSet(const std::string& filename)
{
    ...
    while (getline(file, ligne)) {
        ...
        while(getline(iss, token, ','))
            message += to_string(std::stod(token)) + ",";
        ...
        */
        ...
        if (m_TrainingSet.GetInputNb() <= 0)
            m_TrainingSet = CData(data.size(), target.size());
        m_TrainingSet.AddData(data, target);
    }
...
void Raven_Game::ClickLeftMouseButton(POINTS p)
{
  ...
  if (m_pSelectedBot && m_pSelectedBot->isPossessed())
  {  
      if ((m_TrainingSet.GetInputSet().size() < 2500)) {
          debug_con << "ajout data de tir" << "";
